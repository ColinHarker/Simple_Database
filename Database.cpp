...
int Database::create(string n, string e, string p)
{
	...
	if (head == nullptr)
	{ //if there is nothing yet in the list, create node and add it to list
		newNode = new node;
	...
	{ //if there is already and element in the list
...
int Database::insertElement(int i, string n, string e, string p)
{
	...
	{ //check to see if the database has been initialized
		...
		case 2: //insert element at back of list if there is only one element of the list
			...
			{ //insert element at back if there is more than one element
				tail->next = newNode;
				newNode->next = nullptr;
	...
	{ //if there is no element at the head, then there are no elements in the list
